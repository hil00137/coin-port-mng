...
@Entity
@SequenceGenerator(
    name = Sequence.HOUR_SNAPSHOT + "_gen",
    sequenceName = Sequence.HOUR_SNAPSHOT,
    initialValue = 1,
    allocationSize = 1
)
@DynamicInsert
...
@Table(name = "hour_snapshot", indexes = [Index(name = "idx_hour_snapshot", columnList = "access_info_seq, time")])
data class HourSnapshot(
    @Id
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = Sequence.HOUR_SNAPSHOT + "_gen")
    val seq: Long? = null,
